...
class ShoppingCart:
    ...
    def remove_item(self, product_name, quantity, products):
        if product_name in self.items:
            if quantity <= self.items[product_name]:
                ...
                print("You are trying to remove more items than are in the cart.")
        else:
    ...
    def view_cart(self, products):
        ...
        for name, quantity in self.items.items():
            ...
            subtotal += total
            print(f"{name}: {quantity} x ${product.price:.2f} = ${total:.2f}")
...
class POS_System:
    ...
    def checkout(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                name = input("Enter product product_number: ").strip()
                if name not in self.products:
                    print("Product not found!")
                    continue
                try:
                    quantity = int(input("Enter quantity: "))
                    self.cart.add_item(self.products[name], quantity)
                except ValueError:
    ...
    def generate_receipt(self, subtotal, discount, tax, total, received, change):
        ...
        print("-------------------------------")
        for name, quantity in self.cart.items.items():
            product = self.products[name]
            line_total = quantity * product.price
            print(f"{name}: {quantity} x ${product.price:.2f} = ${line_total:.2f}")
        print("-------------------------------")
        print(f"Subtotal       : ${subtotal:.2f}")
        print(f"Discount       : -${discount:.2f}")
        print(f"Sales Tax (10%): ${tax:.2f}")
